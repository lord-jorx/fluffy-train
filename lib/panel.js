...
export function buildPanel(overrides = {}) {
  const debaters = DEFAULT_DEBATERS.map((d) => ({ ...d }));
  const judge = { ...DEFAULT_JUDGE };

  // Per-request panel overrides win; otherwise fall back to env; otherwise defaults.
  const envModels = (process.env.DEBATER_MODELS || '')
    .split(',')
    .map((m) => m.trim())
    .filter(Boolean);
  const models = Array.isArray(overrides.debaterModels) && overrides.debaterModels.length
    ? overrides.debaterModels
    : envModels;
  models.forEach((model, i) => {
    if (debaters[i] && model) debaters[i].model = String(model).trim();
  });

  const judgeModel = overrides.judgeModel || process.env.JUDGE_MODEL;
  if (judgeModel) judge.model = String(judgeModel).trim();
