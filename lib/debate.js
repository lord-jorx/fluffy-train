...
export async function runDebate(question, emit, reqConfig, signal) {
  ...
  for (const round of ROUNDS) {
    ...
    const results = await mapLimit(debaters, mock ? 5 : cfg.concurrency, async (debater) => {
      ...
      try {
        ...
        const text = mock
          ? await mockTurn(debater, round, onDelta, signal)
          : backend === 'local'
            ? await localTurn(cfg, turnArgs, signal)
            : backend === 'api'
              ? await apiTurn(anthropic, turnArgs, signal)
              : await claudeTurn(turnArgs, signal);
  ...
  try {
    ...
    if (mock) {
      ...
      if (backend === 'local') await localTurn(cfg, verdictArgs, signal);
      else if (backend === 'api') await apiTurn(anthropic, verdictArgs, signal);
      else await claudeTurn(verdictArgs, signal);
    }
